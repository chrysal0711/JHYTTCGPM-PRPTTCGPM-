...
while keep_continuation
    ...
    while keep_going 
        ...
        % - Luvduv < sigma*betas*max(0.001,min(0.8,NormFz))*dudv 
        while - Luvduv < sigma*betas*(normFz/(max(8,normFz)+min(0.001,normFz)))*dudv && betas > 10^-10 
            % -Luvduv < sigma*betas*dudv
            betas = 0.4*betas;   % betas = 0.5*betas;
            Lu = min(u+betas*du,gradu+betas*Bdu);
            Lv = min(v+betas*dv,gradv+betas*Bdv);
            normFz = sqrt(Lu(:)'*Lu(:) + Lv(:)'*Lv(:));
            Luvduv = Lu(:)'*du(:) + Lv(:)'*dv(:);
        end
        lambda = -1.9*Luvduv*betas/normFz^2;  % lambda = - 1.6*Luvduv*betas/(Lu(:)'*Lu(:)+Lv(:)'*Lv(:));
        old_u = u; 
        old_v = v;
        u = old_u - lambda * Lu;
        v = old_v - lambda * Lv;
...
%         sku = u-old_u;
%         skv = v-old_v;
        old_normF = old_Lu(:)'*old_Lu(:)+old_Lv(:)'*old_Lv(:);
        FkFk0=Lu(:)'*old_Lu(:)+Lv(:)'*old_Lv(:);
        normdk = sqrt(dudv);
        normyk = sqrt(yku(:)'*yku(:)+ykv(:)'*ykv(:));
%       ykdk = yku(:)'*du(:)+ykv(:)'*dv(:);
        Fkdk = Lu(:)'*du(:)+Lv(:)'*dv(:);
         Fkyk = Lu(:)'*yku(:)+Lv(:)'*ykv(:);
%        Fkdk = Lu(:)'*du(:)+Lv(:)'*dv(:);
%        yksk = yku(:)'*sku(:)+ykv(:)'*skv(:);
        NormF = Lu(:)'*Lu(:) + Lv(:)'*Lv(:); % compute ||F(z)||
%        Fk0dk = old_Lu(:)'*du(:)+old_Lv(:)'*dv(:);
             seg=0.01;
             mu=0.5;                                                                                  %0.1  和1没有区别  就继续压小  0.05稍微慢一点
             nu=0.01;                                                                                 %0.1  和1没有区别  就继续压小  0.05稍微慢一点
             if  (abs(NormF-FkFk0)/(max(old_normF,nu*normyk*normdk)))*(Fkdk)<mu*(NormF)                    %限制条件变得更严格 
                  betak=(abs(NormF-FkFk0)/(max(old_normF,nu*normyk*normdk)));
                  du = - Lu + (seg*(Fkyk)/(normyk))*yku + betak*du;
                  dv = - Lv + (seg*(Fkyk)/(normyk))*ykv + betak*dv;  
             else
                  betak=seg*(Fkyk)/(normyk);  
                  du = - Lu + betak*yku;
                  dv = - Lv + betak*ykv;  
             end
        if compute_mse
