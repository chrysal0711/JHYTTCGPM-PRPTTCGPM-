...
while keep_continuation
    ...
    while keep_going 
        ...
        while - Luvduv < sigma*betas*max(0.001,min(0.8,normFz))*dudv && betas > 10^-10 
...
%         sku = u-old_u;
%         skv = v-old_v;
        old_normF = sqrt(old_Lu(:)'*old_Lu(:)+old_Lv(:)'*old_Lv(:));
%         normdk = sqrt(dudv);
...
%        yksk = yku(:)'*sku(:)+ykv(:)'*skv(:);
        normF = sqrt(Lu(:)'*Lu(:) + Lv(:)'*Lv(:)); % compute ||F(z)||
        normF1 = Lu(:)'*old_Lu(:) + Lv(:)'*old_Lv(:);
        Fk0dk = old_Lu(:)'*du(:)+old_Lv(:)'*dv(:);
                        mu=0.5;                                                                                  %0.1  和1没有区别  就继续压小  0.05稍微慢一点
                        theta=(abs(normF1))/(normF*old_normF);
                        if  -Fk0dk>ykdk
                            seg=-Fk0dk;
                        else
                            seg=ykdk;
                        end
                        if  Fkdk<mu*seg                         %限制条件变得更严格 
                            betak=(theta*((normF)^2-(normF)/(old_normF)*abs(normF1))+(1-theta)*((normF)^2))/(seg);    
                        else
                        betak=0;
                        end
                        du = - Lu + betak*du;
