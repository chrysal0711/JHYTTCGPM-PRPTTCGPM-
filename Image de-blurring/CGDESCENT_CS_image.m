function [x,x_debias,objective,times,debias_start,mses,taus]= ...
    CGDESCENT_CS_image(y,A,tau,varargin)
%
% SGCS version 1.0, December 3, 2009
%
% This function solves the convex problem 
% arg min_x = 0.5*|| y - A x ||_2^2 + tau || x ||_1
% using the algorithm SGCS, described in the following paper
%
%
% 
% -----------------------------------------------------------------------
% Copyright (2009): Yunhai Xiao, and Junfeng Yang
% ----------------------------------------------------------------------
%
%
%  ===== Required inputs =============
...
while keep_continuation
    ...
    while keep_going 
      ...
      if (iter > 1)
         ...
         Betak = Fkyk1/Dkyk1-2.0*Fkdk1*Ykyk1/Dkyk1;
         Betak = max(0,Betak);
      end
      %
      old_gradu = gradu;
      ...
      while - Luvduv < sigma*betas*dudv 
          betas = 0.01*betas;  % betas = 0.01*betas;
          Lu = min(u+betas*du,gradu+betas*Bdu);
          Lv = min(v+betas*dv,gradv+betas*Bdv);
          Luvduv = Lu(:)'*du(:) + Lv(:)'*dv(:);
      end
