...
while keep_continuation
    ...
    while keep_going 
        ...
        lambda = -1.6*Luvduv*betas/normFz^2;  % lambda = - 1.6*Luvduv*betas/(Lu(:)'*Lu(:)+Lv(:)'*Lv(:));
        old_u = u; 
        old_v = v;
        u = old_u - lambda * Lu;
        v = old_v - lambda * Lv;
        ...
        ykdk = yku(:)'*du(:)+ykv(:)'*dv(:);
        wk = max(2*normdk*normyk,max(ykdk,old_normF));
        Fkyk = Lu(:)'*yku(:)+Lv(:)'*ykv(:);
        Fkdk = Lu(:)'*du(:)+Lv(:)'*dv(:);
        yksk = yku(:)'*sku(:)+ykv(:)'*skv(:);
        tk = min(0.3,max(0,1-yksk/normyk^2));
        betak = Fkyk/wk-normyk^2*Fkdk/wk^2;
        thetak = tk*Fkdk/wk;
        du = - Lu + betak*du + thetak*yku;
        dv = - Lv + betak*dv + thetak*ykv;  
