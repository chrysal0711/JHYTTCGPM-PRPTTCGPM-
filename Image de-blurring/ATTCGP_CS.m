function [x,x_debias,objective,times,debias_start,mses,taus]= ...
    ATTCGP_CS(y,A,tau,varargin)
%
% ATTCGP_CS 1.0, Dec., 16, 2019
%
...
%       NormF = sqrt(NormF); 
      if (iter > 1)
         ...
         skv = v-old_v;
         % compute y_{k}
         yku = Lu-old_Lu+0.1*sku; 
         ykv = Lv-old_Lv+0.1*skv;
         % compute sk'*yk, ||sk||^2 and ||yk||^2
         skyk = sku(:)'*yku(:)+skv(:)'*ykv(:);
         Normsk2 = sku(:)'*sku(:)+skv(:)'*skv(:);
         Normyk2 = yku(:)'*yku(:)+ykv(:)'*ykv(:);
         % compute tkb
         tkb  = max(2-skyk^2/(Normsk2*Normyk2)-skyk/Normsk2,0);
         % compute F_{k}'*d_{k} and |F_{k}'*d_{k}|
         Fkdk = old_Lu(:)'*old_du(:)+old_Lv(:)'*old_dv(:);
         AFkdk = abs(Fkdk);
         % compute F_{k+1}'*yk and F_{k+1}'*sk
         Fk1yk = Lu(:)'*yku(:)+Lv(:)'*ykv(:);
         Fk1sk = Lu(:)'*sku(:)+Lv(:)'*skv(:);
         % compute thetak
         thetak = skyk/Fkdk;
         % compute betak
         Betak = Fk1yk/Fkdk+tkb*Fk1sk/Fkdk; 
         % compute lambdak
         lambdak = Fk1sk/Fkdk;
         % computation of search direction vector
         if (AFkdk>0.1*Normsk2 || AFkdk<0.01*Normsk2) %mu1=0.01;mu2=0.1;
             du = -Lu;
             dv = -Lv;
         else
             du = - thetak*Lu + Betak*sku - lambdak*yku;
             dv = - thetak*Lv + Betak*skv - lambdak*ykv;
         end
      else
          du = - Lu;
          dv = - Lv;
...
%       old_gradv = gradv;
      ...
      sigma = 0.0001;
      betas = 1; % betas = 1; 0.8 better than 1
      ...
      while - Luvduv < sigma*betas*dudv 
          betas = 0.6*betas;  % betas = 0.6*betas;
