...
% NO=1;
% method='PRP+';
 function [Tcpu,NF,Itr,NG] = Mline(NO,method) 
format long
Step=0:4;
nextstep=Step(1);
% disp(date);
                ...
                if norm(Fk,2)<=epsilon || Itr>2000  %跳出循环
                 ...
                 %% %%% Start Armijo-type line search  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    ...
                    while L1==1
                        ...
                        if (-Fz_new'*dk < sigma*t*min(0,norm(Fz_new,2))*norm(dk,2)^2 && t>10^(-10))  % the Armijo-type line search condition violated
                            L1=1;
                            t=t*rho;
                        else
                            L1=0;
...
%                     end
            ...
            case Step(4)
                ...
                switch method
                    ...
                    case 'MN'
                        mu=1.4;
                        betak=(norm(Fk)^2-max(Fk'*Fk0,max(0,norm(Fk)/norm(Fk0)*Fk'*Fk0)))/(max(mu*norm(dk)*norm(Fk),dk'*yk));%max(norm(Fk0)^2,
                        dk=-1*Fk+betak*dk; 
                    case 'YJH'
                        mu=1.4;
                        betak=(norm(Fk)^2-max(0,norm(Fk)/norm(Fk0)*Fk'*Fk0))/max(mu*norm(dk)*norm(Fk),dk'*yk);
                    ...
                    case 'JYJ' %投JCAM使用的共轭参数
                        mu=1.4;
                        %betak=min(abs(Fk'*(Fk-Fk11))/norm(Fk11)^2,(norm(Fk)^2-max(0,norm(Fk)/norm(Fk11)*Fk'*Fk11))/(mu*norm(dk)*norm(Fk)));
                    ...
                    case 'YLS'
                        ...
                        dk=-1*betak*Fk;
                    case 'WZ' %2018 凸约束非线性方程组的一种无导数投影方法_吴晓云《数学的实践与认知》
                        tk=1+max(0,-dk'*yk/(dk'*dk));
                        yk1=yk+tk*dk;
                        betak=(yk1-dk-norm(yk1)^2/(dk'*yk1)*dk)'*Fk/(dk'*yk1); 
                        dk=-1*Fk+betak*dk; 
                   ...
                   case'TTCGM'  % Gao, P., & He, C. (2018). An efficient three-term conjugate gradient method for nonlinear monotone equations with convex constraints. 
                       % Calcolo, 55(4), 53.
                        betak=-Fk'*Fk0/(Fk0'*dk);
                        thetak=Fk'*dk/(Fk0'*dk);
                        dk=-Fk+betak*dk+thetak*Fk0;
                   case'DFCGP' % Sun M, Tian M. A Class of Derivative-Free CG Projection Methods for Nonsmooth Equations with an Application to the LASSO Problem. 
                       % Bulletin of the Iranian Mathematical Society, 2019: 1-23.
