...
% NO=1;
% method='PRP+';
 function [Tcpu,NF,Itr,NG] = line1(NO,method) 
format long
Step=0:4;
nextstep=Step(1);
% disp(date);
            ...
            case Step(4)
                ...
                switch method
                    ...
                    case 'JYJ' %投JCAM使用的共轭参数
                        ...
                        dk=-1*Fk+betak*dk;
                    case 'YLS' %Yu Z, Lin J, Sun J, et al. Spectral gradient projection method for monotone nonlinear equations with convex constraints[J]. 
                        % Applied numerical mathematics, 2009, 59(10): 2416-2423.
                        r=0.01;
                        sk=x1-x0;
                        yk1=yk+r*sk;
                        betak=norm(sk)^2/(sk'*yk1);
                        dk=-betak*Fk; 
                   case'ATTCGP' %Gao P, He C, Liu Y. An adaptive family of projection methods for constrained monotone nonlinear equations with applications[J]. 
                       % Applied Mathematics and Computation, 2019, 359: 1-16.
                       r=0.1;
                       sk=x1-x0;
                       yk1=yk+r*sk;
                       tkb=max(2-(sk'*yk1)^2/(norm(sk)^2*norm(yk1)^2)-sk'*yk1/norm(sk)^2,0);
                       if abs(Fk0'*dk)>0.1*norm(sk)^2 || abs(Fk0'*dk)<0.01*norm(sk)^2
                           dk=-Fk;
                       else
                           dk=sk'*yk1/(Fk0'*dk)*Fk+(Fk'*yk1/(Fk0'*dk)+tkb*Fk'*sk/(Fk0'*dk))*sk-Fk'*sk/(Fk0'*dk)*yk1;
                       end
                end
                x0=x1;
                if norm(dk)<epsilon1
