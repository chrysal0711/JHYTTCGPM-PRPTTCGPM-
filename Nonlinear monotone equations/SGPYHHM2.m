...
% NO=1;
% method='PRP+';
 function [Tcpu,NF,Itr,NG] = SGPYHHM2(NO,method) 
format long
    ...
    while loop==1
        switch nextstep
            case Step(1)
                ...
                gamma=1.8;    % gamma=1, initial guess for the steplength
                rho=0.4;    % rho=0.9
                %% 初始方向
                 ...
                 %% Start Armijo-type line search  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    ...
                    while L1==1
                        ...
                        NF=NF+1;
                        eta_k=(norm(Fz_new,2))/(max(8,norm(Fz_new,2))+min(0.001,norm(Fz_new,2)));  % 0.001,8
...
%               Fk0=feval(nprob,n,x0,1);
                ...
                switch method
                     case 'PRPTTCGPM2'       
                        ...
                        nu=0.01;
                        if  (abs(Fk'*Fk-Fk'*Fk0)/(max(Fk0'*Fk0,nu*norm(dk,2)*norm(yk,2))))*(Fk'*dk)<mu*(Fk'*Fk)                    %限制条件变得更严格 
                        betak=(abs(Fk'*Fk-Fk'*Fk0)/(max(Fk0'*Fk0,nu*norm(dk,2)*norm(yk,2))));      
                        dk=(-1)*Fk+betak*dk+(seg*(Fk'*Fk0)/(Fk0'*Fk0))*Fk0;
                        else
                        betak=seg*(Fk'*Fk0)/(Fk0'*Fk0);  
                        dk=(-1)*Fk+betak*Fk0;  
                        end
                    case 'MDYKDCGP'       
                        yita=0.01;
                        nu=0.5;                                                                                  %0.1  和1没有区别  就继续压小  0.05稍微慢一点
                        if  Fk'*dk<-nu*(Fk0'*dk)                          %限制条件变得更严格 
                            if (Fk'*Fk)/(-dk'*Fk0)<abs((Fk'*Fk)/(dk'*yk))
                            betak=(Fk'*Fk)/(-dk'*Fk0);    
                            dk=(-1)*Fk+betak*dk;
                            else
                            betak=abs((Fk'*Fk)/(dk'*yk));    
                            dk=(-1)*Fk+betak*dk;
                            end
                        else
                        betak=yita*(Fk'*dk)/(dk'*dk);   
                        dk=(-1)*Fk+betak*dk;  
                        end
                    case 'WYL'
